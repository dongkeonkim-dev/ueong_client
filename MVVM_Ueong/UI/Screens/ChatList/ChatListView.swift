  ...
  var body: some View {
    VStack {
      ...
      .padding(.horizontal, 20)
        ...
        // Chat List
      List(viewModel.chats.sorted(by: {
          // 두 날짜 모두 nil이 아닌 경우에만 비교
        if let date1 = $0.lastSentTime, let date2 = $1.lastSentTime {
          return date1 > date2
        }
          // date1이 있고 date2가 nil이면 date1이 더 최근
        else if $0.lastSentTime != nil {
          return true
        }
          // date2가 있고 date1이 nil이면 date2가 더 최근
        else if $1.lastSentTime != nil {
          return false
        }
          // 둘 다 nil이면 순서 유지
        else {
